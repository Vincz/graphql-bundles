...
abstract class MetadataConfiguration
{
    ...
    abstract public function setClassesMap(ReflectionClass $reflectionClass, Meta $annotation): void;

    abstract public function getConfiguration(ReflectionClass $reflectionClass, Meta $annotation): array;
    ...
    {
        $config = [];
        $descriptionAnnotation = $this->getFirstMetadataMatching($metadatas, Metadata\Description::class);
        if (null !== $descriptionAnnotation) {
            $config['description'] = $descriptionAnnotation->value;
        }

        if ($withDeprecation) {
            $deprecatedAnnotation = $this->getFirstMetadataMatching($metadatas, Metadata\Deprecated::class);
